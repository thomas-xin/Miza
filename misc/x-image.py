...
import os, sys, io, time, concurrent.futures, subprocess, psutil, asyncio, collections, traceback, re, requests, blend_modes, pdf2image, zipfile, contextlib, filetype, pyqrcode, ast, colorspace, pickle
import numpy as np
import PIL
            ...
            if fmt == "zip":
                ...
                if fmt in ("gif", "webp", "apng"):
                    ...
                    if new.get("count", inf) <= 16:
                        ...
                        crf = max(17, min(51, round(log(np.prod(size), 2) * 6 - 92)))
