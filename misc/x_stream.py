			...
			for i, (start, end) in enumerate(ranges):
				...
				while rems:
					...
					if isinstance(u, byte_like):
						...
						resp = await _run_async(niquests.head, u, timeout=3)
					...
					fut = _run_async(get_chunk, u, headers, start, end, pos, ns)
					futs.append(fut)
					pos = 0
					start = 0
					end -= start + ns
...
async def unproxy(request: Request, path: Optional[str] = None, url: Optional[str] = None, force: bool = False, download: bool = False):
	"""Reverse proxy Discord attachments or redirect to direct URLs."""
...
async def proxy(request: Request, url: Optional[str] = None, force: bool = False, download: bool = False):
	...
	if not force and heads.get("content-type").split(";", 1)[0] == "text/markdown":
		...
		return Response(
			"""<!doctype html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><script src="https://cdn.jsdelivr.net/npm/marked/marked.min.js"></script><style>:root{--c0:#fff;--c1:#111;--c2:#eee;--c3:#333}._dt{--c0:#111;--c1:#fff;--c2:#333;--c3:#eee}body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;line-height:1.6;max-width:800px;margin:0 auto;padding:20px;color:var(--c1);background-color:var(--c0)}#viewer blockquote{border-left:4px solid #ccc;margin-left:0;padding-left:16px;color:var(--c3)}#viewer code{padding:2px 4px;border-radius:4px;background-color:var(--c2)}#viewer pre{padding:16px;border-radius:4px;overflow-x:auto;background-color:var(--c2)}</style></head><body><div id="viewer"><p><em>Loading...</em></p></div><script>async function renderMarkdown() {if (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) {document.body.classList.add('_dt');}try {const r = await fetch(URL);if (!r.ok) throw new Error(`HTTP error! status: ${r.status}`);const e = await r.text();viewer.innerHTML = marked.parse(e);} catch (r) {viewer.innerHTML = `<p style="color: red;"><strong>Failed to load Markdown:</strong> ${r.message}</p>\n<p><small>Note: The server hosting the file must allow Cross-Origin Resource Sharing (CORS).</small></p>`;}}renderMarkdown();</script></body></html>""".replace("URL", json.dumps(new_url)),
			headers=response_headers,
...
async def static_backend(path: str, request: Request):
	...
	async def backend_request(url):
		...
		print(url, headers)
		resp = await _run_async(server.session.get, url, headers=dict(headers), verify=False, timeout=60)
