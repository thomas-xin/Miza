...
class Bot:
	...
	def question_context_analysis(self, m, q, c):
		...
		inputs = tokenizer(q[:512], c[:512], return_tensors="pt", max_length=4096)
		with torch.no_grad():
	...
	def question_answer_analysis(self, m, q, qh, ah):
		...
		new_user_input_ids = tokenizer.encode(q + end, return_tensors="pt", max_length=4096)
		history = []
		for k, v in self.history.items():
			history.append(tokenizer.encode(k + end, return_tensors="pt", max_length=4096))
			if v:
				history.append(tokenizer.encode(v + end, return_tensors="pt", max_length=4096))
