...
class FileSystem(FileHashDict):
	storage = ChunkSystem(path=TEMP_PATH + "/chunks", automut=False, autosave=60)

	def __init__(self, *args, path=TEMP_PATH + "/filehost", **kwargs):
		super().__init__(*args, path=path, automut=False, autosave=30, safe=True, **kwargs)
		self.lock = threading.Lock()
		self.work_queue = self.coercedefault("@work_queue", alist, alist())
		# TODO: Actually use hashes for dedup
		self.hash_table = self.setdefault("@hash_table", {})

	@classmethod
	def analyse_chunks(cls, chunks, id=0):
		output = []
		mime = None
		size = 0
		def create_chunk(buffer):
			nonlocal output, mime, size
			fn = f"{cls.path}/{id}-{size}"
			with open(fn, "wb") as f:
				for data in buffer:
					f.write(data)
				size += f.tell()
		...
		for chunk in chunks:
			if isinstance(chunk, str):
				if is_url(chunk):
					if bufsize:
						...
						bufsize = 0
					length = None
					resp = requests.head(chunk, headers=Request.header())
					if not mime:
						try:
							mime = resp.headers["Content-Type"]
						except KeyError:
							resp = requests.get(chunk, headers=Request.header())
							mime = get_mime(resp.content)
					...
					if not length:
						try:
							...
							try:
								length = resp.headers["x-goog-stored-content-length"]
							except KeyError:
								length = cls.max_size
					output.append(chunk)
					size += length
			...
			if isinstance(chunk, io.BytesIO):
				chunk = chunk.getbuffer() # goto byte_like case
			if isinstance(chunk, io.IOBase):
				chunk.seek(0)
				chunk = chunk.read()
			if isinstance(chunk, byte_like):
				while bufsize + len(chunk) > cls.max_size:
					chunk = astype(chunk, memoryview)
					cut = cls.max_size - bufsize
					left, chunk = chunk[:cut], chunk[cut:]
					buffer.append(left)
					create_chunk(buffer)
					buffer.clear()
					bufsize = 0
				...
				raise NotImplementedError(type(chunk))
		if bufsize:
			create_chunk(buffer)
		return output, mime, size

	def create(self, chunks, id=None, name="Untitled", mime="application/octet-stream", key=None, owner=None, private=False):
		if id is None:
			id = ts_us()
			with self.lock:
				while id in self:
					id += 1
		chunks, mime, size = self.analyse_chunks(chunks, id=id)
		if hash is None:
			for chunk in chunks:
				if 0:
					pass
	...
	def upload(self, f, owner=None):

		file = self.create()

	def retrieve(self, id, key=None):
		file = self[id]
		if file.get("private") and key != file.get("key"):
