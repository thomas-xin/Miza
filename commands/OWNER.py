...
class Reload(Command):
	...
	async def __call__(self, bot, _channel, _message, unload, reload, **void):
		...
		if unload:
			...
			succ = await _run_async(bot.unload, unload)
			if succ:
		...
		if reload:
			...
			succ = await _run_async(bot.reload, reload)
			if succ:
...
class Restart(Command):
	...
	async def __call__(self, bot, _message, _channel, _user, mode, delay, **void):
		...
		if mode == "audio":
			...
			await _run_async(bot.start_audio_client)
	...
	async def confirm_shutdown(self, _channel=None, _user=None, mode="restart"):
		...
		async with Delay(1):
			async with ctx:
				...
				with tracebacksuppressor:
					await _run_async(bot.update, force=True)
...
class UpdateExec(Database):
	...
	async def procFunc(self, message, proc, bot, term=0):
		...
		if term & 32:
			...
			return output
		if self.psem.busy:
			output = await _run_async(aeval, proc, glob)
		else:
	...
	async def uproxy(self, *urls, collapse=True, mode="upload", filename=None, channel=None, optimise=False, **kwargs):
		...
		async def proxy_url(url):
			...
			if isinstance(url, byte_like):
				...
				data = await _run_async(url.fp.read)
