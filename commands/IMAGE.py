    ...
    async def __call__(self, bot, guild, user, channel, message, name, args, flags, comment="", **void):
        ...
        if amount2 < amount:
            ...
            with discord.context_managers.Typing(channel):
                ...
                #         fut = create_task(process_image("IBASL", "&", [prompt, kwargs, nsfw, False, n], fix=3, pwr=1000000, timeout=120))
                #         futt.append(fut)
                #         c2 -= n
                ...
                while amount2 < amount:
                    ...
                    async with self.sdiff_sem:
                        ...
                        if self.has_py39:
                            ...
                            while c2 > 0:
                                n = min(c2, xrand(floor(sqrt(c2) + 1)) + 1)
                                fut = create_task(process_image("IBASL", "&", [p, kwargs, nsfw, False, n], fix=3, pwr=1000000, timeout=120))
                                futt.append(fut)
                                c2 -= n
                            for fut in futt:
                                ims = await fut
