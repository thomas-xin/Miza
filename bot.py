...
class Bot(discord.Client, contextlib.AbstractContextManager, collections.abc.Callable):
    ...
    def is_mentioned(self, message, user, guild=None):
        ...
        if guild is None:
            return True
        if "exec" in self.data and self.data.exec.get(message.channel.id, 0) & 64 and message.content and message.content[0] not in "\\#":
            return True
        member = guild.get_member(u_id)
        if member is None:
            return False
    ...
    def clear_cache(self):
        ...
        with self.cache_semaphore:
            ...
            while stats.free < 81 * 1073741824 or len(expendable) > 4096 or (t - expendable[0].stat().st_atime) > 3600 * 6:
                with tracebacksuppressor:
