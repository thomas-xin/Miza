...
class Bot(discord.AutoShardedClient, contextlib.AbstractContextManager, collections.abc.Callable):
	...
	def init_main_databases(self):
		...
		self.emoji_animated = AutoCache(f"{CACHE_PATH}/emoji_animated", stale=86400 * 365, timeout=None, desync=0.05)
	...
	def fetch_member(self, u_id, guild=None, find_others=False):
		...
		return _run_async(self.get_member, u_id, guild, find_others)
	...
	async def fetch_message(self, m_id, channel=None, old=False, fast=False):
		...
		if not m and "message_cache" in self.data:
			with suppress(KeyError):
				m = await _run_async(self.data.message_cache.load_message, m_id)
		if m and not old:
			if m.attachments:
	...
	async def browse(self, argv, uid=0, timezone=None, region=None, n=3, timeout=60):
		...
		if argv:
			urls.append(argv)
		futs = [create_task(self.extract_cache.aretrieve(
	...
	async def to_data_url(self, url, small=False, fmt="jpg", timeout=8):
		...
		return await self.extract_cache.aretrieve(tup, self._to_data_url, url, small, fmt, timeout)
	...
	async def autodelete(self, *messages, keep_log=True, channel=None):
		if not messages:
			return
		collections = {}
		futs = []
	...
	async def get_system_stats(self):
		...
		def count_size():
			...
			return size
		size = await _run_async(count_size)
	...
	async def init_ready(self):
		await _run_async(self.start_webserver)
		await self.modload
	...
	def set_client_events(self):
		...
		async def on_full_connect():
			await _run_async(self.set_guilds)
		...
		async def on_guild_join(guild):
			...
			if guild.me.guild_permissions.view_audit_log:
				...
				with suppress(discord.Forbidden):
					a = guild.audit_logs(limit=100, action=discord.AuditLogAction.bot_add)
					async for e in a:
		...
		async def on_raw_message_delete(payload):
			with tracebacksuppressor:
				...
				await fut
				user = None
				if self.get_guildbase(guild.id, "logs.message") and guild.me.guild_permissions.view_audit_log and not getattr(message, "ghost", None):
					delay = 1 - (utc() - start)
					await asyncio.sleep(delay)
					t = (message.author.id, message.channel.id)
			...
			with tracebacksuppressor:
				...
				async with asyncio.TaskGroup() as group:
					for message in messages:
						...
						if guild:
							fut = self.send_event("_delete_", message=message, bulk=True)
							group.create_task(fut)
				if self.get_guildbase(guild.id, "logs.message") and guild.me.guild_permissions.view_audit_log:
...
class SimulatedMessage:
	...
	async def send(self, *args, **kwargs):
		...
		try:
			...
			ofiles = []
			for file in files:
				f = await _run_async(self.as_file, file)
				ofiles.append(f)
			kwargs["files"] = ofiles
